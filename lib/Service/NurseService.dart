...
class NurseService {
  Future<List<Nurse>> getNurse() async {
    final response = await http.get(Uri.https(Strings.pathAPI, 'api/Nurse'));
  ...
  Future<bool> InsertNurse(Nurse nurse) async {
    ...
    final response = await http.post(Uri.https(Strings.pathAPI, 'api/Nurse'),
  ...
  Future<bool> UpdateNurse(NurseModel nurseModel) async {
    final response = await http.put(
        Uri.https(Strings.pathAPI, 'api/Nurse/${nurseModel.id}'),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
        },
        body: jsonEncode(nurseModel.toJson()));
